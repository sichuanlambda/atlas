...
## Building Submission — Direct DB Pipeline (v3, Feb 20)
- **Playwright is DEAD** — direct DB via `heroku run -- rails runner -` heredoc
- 9 buildings in 30 seconds vs 90s/building with browser. Scales to 50+ per run.
- Pattern: BuildingAnalysis.new(..., visible_in_library: true).save!(validate: false) → S3 upload → ProcessBuildingAnalysisJob.perform_later
- **MUST set visible_in_library: true** explicitly or buildings don't show on Place pages
- S3 upload from VPS: boto3 + AWS creds from `heroku config --json`
- S3 upload from Heroku: URI.open → bucket.object(key).upload_file
- Full procedure: memory/runbooks/building-submission.md

## Typefully Integration (Feb 20)
- API key: ~/.config/secrets/typefully.env
- Social sets: @ArchitectrHelpr (207404, auto), @CRESoftware (207403, auto), @NathanSRobinson (205514, drafts only), @PlotzyAI (205524, future)
- LinkedIn: Nathan's personal + Plotzy only. NOT AH or CRE.
- Image upload: POST media/upload → PUT to presigned URL (NO Content-Type header) → wait 15s → media_id
- Threads: multiple posts in posts[] array, each with media_ids[]
- Scheduling: "next-free-slot", "now", or ISO datetime
- Style guides: memory/social-style-guides.md
- Content system: memory/content-system.md
- Daily content cron: 14:00 UTC, job 07e3b369

## Critical Public-Facing Rules
- NEVER say "GPT analysis" — use "analysis", "style analysis", "breakdown"
- Building URLs: app.architecturehelper.com/architecture_explorer/{id} (NOT /building_analyses/)
- Domain: app.architecturehelper.com (NOT architecturehelper.com — 404s)
- CRE URLs: cresoftware.tech/category.html#slug (NOT /#/category/)

## Architecture Helper — Current State (updated 2026-02-20)
- 13 cities with Place pages: Denver, Den Haag, SF, Chicago, NOLA, NYC, DC, Philly, Boston, Barcelona, Edinburgh, Bruges, Kraków
- ~1105 total buildings in database
- Place pages require thorough editorial content (4000-6000+ chars, 5 sections with h2/h3)
